...
- **Employees**: open the site on their phone's browser → Login/Logout with
  a motion-detection selfie (blink/smile/turn prompt) + location → view
  their own attendance calendar with hours spent → raise a mistaken-logout
  request.
...
- **Manager**: approve/reject regularization requests (capped at 3/month per
  employee — beyond that, email Admin directly), view their direct reports'
  attendance calendar + CSV export.

See **DEPLOYMENT.md** for the full setup walkthrough (takes about 20 minutes
end to end: create Supabase project → run 3 SQL files → deploy 1 small
