...
// profileId (optional): when given, only that person's rows are drawn - a
// safety net so one person's calendar can never show someone else's day.
export default function AttendanceCalendar({ records, month, profileId }) {
  ...
  // Clear the open day when switching month or person.
  useEffect(() => { setSelected(null); }, [year, monthIdx, profileId]);

  const byDate = {};
  (records || [])
    .filter((r) => !profileId || r.profile_id === profileId)
    .forEach((r) => { byDate[r.att_date] = r; });
      ...
      {sel && (
        <div className="cal-detail">
          <div className="cal-detail-header">
            ...
            <span className={`pill ${sel.status}`}>{String(sel.status).replace('_', ' ')}</span>
          </div>
