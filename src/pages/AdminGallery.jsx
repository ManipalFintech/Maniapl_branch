...
import { formatTime, pad, todayIso } from '../lib/time.js';
...
export default function AdminGallery() {
  ...
  useEffect(() => {
    if (!from || !to || from.length < 10) return undefined;
    let active = true;
    setLoading(true);
    setError('');
  ...
  async function handleDownloadZip() {
    ...
    for (const r of filtered) {
      const folder = zip.folder(`${r.profiles?.name || 'Unknown'} (${r.profiles?.emp_id || r.profile_id})`.replace(/[\\/:*?"<>|]/g, '-'));
      const jobs = [];
    ...
    try {
      ...
      api.downloadBlob(content, mode === 'day' ? `attendance_photos_${day}.zip` : `attendance_photos_${month}.zip`);
    } catch (err) {
      setError(`Could not build the ZIP file: ${err.message}`);
    } finally {
      setZipping(false);
      setZipProgress('');
    }
