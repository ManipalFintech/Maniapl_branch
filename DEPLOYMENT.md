...
- [ ] 2–3 pilot employees created, tested full flow on their own phones:
      Login (motion-prompt selfie + location) → Logout → mistaken-logout
...
- **Motion-detection model download:** the blink/smile/turn-head prompt
  loads a small face-detection model from a public CDN the first time
  someone opens the camera each session — needs a normal internet
  connection, a few hundred KB, loads in a couple of seconds on 4G/WiFi.
...
- **If you ever need to change the Edge Function:** edit
  `supabase/edge-functions/admin-employees/index.ts`, then re-run
  `supabase functions deploy admin-employees`.
